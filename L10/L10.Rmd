...
##Comparison of old and new plot without frame - not good for publication

```{r}
par(mfrow=c(2,1))
boxplot( iris$Petal.Length ~ iris$Species, ylab="Petal Length (cm)", xlab="Species") # old one
boxplot( iris$Petal.Length ~ iris$Species,  frame.plot=F ) # new one
```
...
```{r}
par(mfrow=c(1,1))
boxplot( iris$Petal.Length ~ iris$Species,  axes=F) # axes=F removes axes from the plot
```

## Adding the L box shape
```{r}
boxplot( iris$Petal.Length ~ iris$Species,  axes=F)
...
```{r}
boxplot( iris$Petal.Length ~ iris$Species, ylim=c(0,7), axes=F)
box(bty="L")
axis(1,at=1:3,c("Iris setosa","Iris versicolor","Iris virginica"),
...
```{r}
boxplot( iris$Petal.Length ~ iris$Species, ylim=c(0,7), axes=F, col = "lightgray")
box(bty="L")
axis(1,at=1:3,c("Iris setosa","Iris versicolor","Iris virginica"),
        font=3, cex.axis=1.2, tcl=0.5)
title(xlab="Species", cex.lab=1.4)
...
```{r}
boxplot( iris$Petal.Length ~ iris$Species, ylim=c(0,7), axes=F, col=rainbow(10))
box(bty="L")
axis(1,at=1:3,c("Iris setosa","Iris versicolor","Iris virginica"),
        font=3, cex.axis=1.2, tcl=0.5)
title(xlab="Species", cex.lab=1.4)
