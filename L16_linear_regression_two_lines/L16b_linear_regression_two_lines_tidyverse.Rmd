---
title: "Lecture 16b:ANCOVA in tidyverse"
...
This is the `tidyverse` version of "L16 linear regression with two lines (ANCOVA)".
...
library(magrittr)  # for the %<>% piping construct
library(broom)     # for plotting actual model objects
```
...
# import the data using the base-R `read.csv` function. 
...
mydata %<>% mutate(Grazing = factor(Grazing, levels = c("Ungrazed", "Grazed")))
...
g1 <- ggplot(data = mydata, 
             mapping = aes(x = Root, y = Seed, color = Grazing)) + 
  geom_point() + 
...
Our *a priori* assumption here is that both slopes and intercept in Grazed and Ungrazed group differ! This means that we need to estimate a total of 4 coefficients. Specifically, we fit a model of the form:
...
and represents the line for the _**Ungrazed**_ plants.
...
# fit the glm and store the ouput in a new object "model1"
model1 <- glm( Seed ~ Root + Grazing + Root:Grazing, 
               data = mydata )

# note that the above line is the full syntax for the short 
# version. Note the "*" to denote, "include all interactions"
...
When we analyse coefficients we see that there is no evidence for a significant interaction between Root and Grazing which is the coefficient that determines whether there is a difference between the two slopes. The next step might be to specify a more simple model with parallel lines, that differ not in their slopes, but only in their intercepts.

## Plotting the fitted model over the data

One of the things about ggplot is that is it encourages short cuts. While making your life potentially easy, it doesnt lend itself to good error checking. For example, one can very easily let ggplot fit separate models to the data.

The approach below effectively splits the data in to two entirely separate pieces based on the Grazing variable which we have used to assign different colours in the plot and fits two linear models with two error terms $\epsilon_{Grazed}$ and $\epsilon_{Ungrazed}$. This is different to our call to the `glm()` function which does something similar but ties the two groups of data together by calculating a single error term $\epsilon$.

```{r}

# generate a new figure from our basic scatter plot g1 and let 
# geom_smooth() add lines based on the colour grouping
g_2 <- g1 + geom_smooth(method = "lm")

print(g_2)

```
In order to plot the actual models we fitted, we have to actually make reference to our model in the ggplot construct. We do this using some helper functions in the `broom` package which will generate a new `data.frame` object (strictly a `tibble` per the tidyverse realm) based on the data we have and the model fit we created, keep it all in the same structure as the original data and hence allow ggplot to inherit all the information it needs to add it to the plot.

```{r}

model1_fitted <- model1 %>% augment(se_fit = TRUE)

g_model1 <- ggplot(data = model1_fitted, 
                   mapping = aes(x = Root, y = Seed, 
                                 color = Grazing)) + 
  geom_ribbon(aes(ymin=.fitted-1.96*.se.fit, ymax=.fitted+1.96*.se.fit), 
              alpha=0.2, linetype = 0) +
  geom_point() + 
  geom_line(aes(Root, .fitted))
...
```{r}
# store the parallel model in the object "model2"
model2 <- glm( mydata$Seed ~ mydata$Root + mydata$Grazing )

summary(model2)

# extract and store the coefficients into an object called coefmod2
# for easier recall.
...
abline (coefmod2[1],coefmod2[2], col="black", lty=1) 

#Then add the line for the Ungrazed group
abline (coefmod2[1] + coefmod2[3],coefmod2[2], col="red", lty=2 ) 
```
