...
We expect the response variable Seed and the linear covariate Root to both be numerical, but Grazing represents two categories in this experiment and we should convert these to a factor. Since the experimental treatment is designed to look at the effect of Grazing over no grazing, I am going to force the baseline level in this factor to be the "Ungrazed" category. This means that our fitted model later on will expclitly tell us what the effect of grazing is, rather than what the effect of "ungrazing" is which is a bit counter-intuitive.

```{r factor-grazing}

# coerce to factor and specify the order of the levels
mydata$Grazing <- factor(mydata$Grazing, levels = c("Grazed", "Ungrazed"))
...
```{r}
model1 <- glm( Seed ~ Root, data = mydata )
